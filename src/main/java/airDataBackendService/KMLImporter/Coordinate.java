...
public class Coordinate implements Comparable<Object> {
	 ...
	 * @param lat1 Latitude
	 * @param lon1 Longitude
	 * @param H    Hoehe ueber Normalnull
	 */
	 ...
	 * 
	 * @param cord Koordinate zu der die Distanz berechnet wird
	 * @return DIstanz zwischen this und cord in Kilometer
	 */
	...
	/**
	 * Spannt zwischen zwei Koordinaten ein Kugeloberfl�chensegment auf und pr�ft,
	 * ob diese Koordinate sich innerhalb des Segments befindet
	 * 
	 * @param c1 Eckpunkt 1 des Kugeloberfl�chensegments
	 * @param c2 Eckpunkt 2 des Kugeloberfl�chensegments
	 * @return true wenn sich this innerhalb des Segments befindet und false wenn
	 *         nicht.
	 */

	public boolean insideField(Coordinate c1, Coordinate c2) {
		if (this.lat > (Math.max(c1.lat, c2.lat) + 0.002) || this.lon > (Math.max(c1.lon, c2.lon) + 0.002)
				|| this.lat < (Math.min(c1.lat, c2.lat) - 0.002) || this.lon < (Math.min(c1.lon, c2.lon) - 0.002)) {
			return false;
		} else {
			return true;
		}
	 ...
	 * 
	 * @param s Ein String aus der KML-Datei der die Position einer Station
	 *          beschreibt
	 * @return Ein dem Text entsprechendes Koordinaten Objekt
	 */
	 ...
	 * 
	 * @param cord Koordinate mit der die Gleichheit ueberprueft wird
	 * @return true wenn Position und Hoehe gleich, sonst false
	 */
	 ...
	 * 
	 * @param c1 Koordinate mit der die Gleichheit ueberprueft wird
	 * @return true wenn Position gleich, sonst false
	 */
	 ...
	 * 
	 * @return 1 wenn this eine hoehere Latitude bzw. gleiche Latitude und hoehere
	 *         Longitude hat bzw. gleiche Latitude und Longitude und groessere
	 *         Hoehe; 0 wenn exakt gleich; sonst -1
	 */

	@Override
	public int compareTo(Object c1) {
		Coordinate cord = (Coordinate) c1;
		if (this.lat > cord.lat || ((this.lat == cord.lat) && (this.lon > cord.lon))
				|| ((this.lat == cord.lat) && (this.lon == cord.lon) && (this.hight > cord.hight))) {
			return 1;
	 ...
	 * 
	 * @return String der die Koordinate beschreibt
	 */
